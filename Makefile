...
# Copyright: 2011-2012 Matthew T. Pandina
...
# Remove clock divider, set external crystal, enable clock output, BOD 2.7 V
#FUSES      = -U hfuse:w:0xd9:m -U lfuse:w:0xa6:m -U efuse:w:0xFD:m

# ---------- Timers Used ----------
#
# 8-bit Timer/Counter0 or 8-bit Timer/Counter2
#
#  Used by TLC5940_Init()/ISR(TIMER0_COMPA_vect) to call the ISR every
#  2^TLC5940_PWM_BITS clock cycles
#
...
# This defines which pins the diagnostic LED is connected to:
LED_DDR = DDRB
LED_PORT = PORTB
LED_INPUT = PINB
LED_PIN = PB1
